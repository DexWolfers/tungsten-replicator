...
//        if (logger.isDebugEnabled())
        ...
        switch (type) {
            ...
            case MysqlBinlog.MYSQL_TYPE_JSON:
                // printHexString(row);
                int startPos = rowPos + 4;
                int jsonType = BigEndianConversion
                        .convert1ByteToInt(row, startPos);

                if (jsonType == 0 || jsonType == 2) {
                    length = LittleEndianConversion
                            .convert2BytesToInt(row, startPos + 3);
                } else if (jsonType == 1 || jsonType == 3) {
                    length = LittleEndianConversion.convert4BytesToInt(row, startPos + 3);
                }
                try {
                    String jsonStr = parseBinary(jsonType, Arrays.copyOfRange(row, startPos + 1, startPos + length + 1)).toString();
//                            SerialBlob blob = DatabaseHelper.getSafeBlob(row, startPos, length + 1);
//                    logger.error("jsonstr---------------------------------------------------------------------------------"+jsonStr);
                    value.setValue(jsonStr);
                } catch (Exception e) {
                    throw new MySQLExtractException(
                            "Failure while extracting blob", e);
                }
                if (spec != null) {
                    spec.setType(Types.VARCHAR);
                }
                return length + 5;
    ...
    private JsonElement parseBinary(int jsonType, byte[] row) throws MySQLExtractException {
        switch (jsonType) {
            case 0:
                return parseSmallJson(row);
            case 1:
                return new JsonObject();
            case 2:
                return parseSmallArray(row);
            case 3:
    ...
    private Boolean parseLiteral(int value) {
        switch (value) {
            case 0:
                return null;
            case 1:
    ...
    private JsonObject parseSmallJson(byte[] row) throws MySQLExtractException {

        JsonObject obj = new JsonObject();
        try {
            int keyNumber = LittleEndianConversion
                    .convert2BytesToInt(row, 0);
            for (int i = 0; i < keyNumber; i++) {

                int keyIndex = LittleEndianConversion
                        .convert2BytesToInt(row, 4 + 4 * i);
                int keyLength = LittleEndianConversion
                        .convert2BytesToInt(row, 6 + 4 * i);
                String key = processString(row,  keyIndex, keyLength);
                int keyValueType = LittleEndianConversion
                        .convert1ByteToInt(row,  4 + keyNumber * 4 + 3 * i);

                int keyValuePosition = LittleEndianConversion
                        .convert2BytesToInt(row,  5 + keyNumber * 4 + 3 * i);

                if (keyValueType >= 0 && keyValueType <= 3) {
                    int valueLength = LittleEndianConversion
                            .convert2BytesToInt(row, keyValuePosition + 2);
                    obj.add(key, parseBinary(keyValueType, Arrays.copyOfRange(row, keyValuePosition, (valueLength + keyValuePosition))));
                } else if (keyValueType == 4) {
                        obj.addProperty(key, parseLiteral(keyValuePosition));
                } else if (keyValueType == 5 || keyValueType == 6) {
                        obj.addProperty(key, keyValuePosition);
                } else if (keyValueType >= 7 && keyValueType <= 11) {
                    obj.addProperty(key, (Number) parseValue(keyValueType, row));
                } else if(keyValueType == 12){
                    int lengthOffsize = 1;
                    int valueLength = LittleEndianConversion
                            .convert1ByteToInt(row, keyValuePosition);
                    if (valueLength >= 128) {
                        int secondValueLength = LittleEndianConversion
                                .convert1ByteToInt(row, keyValuePosition + 1);
                        valueLength = valueLength + (secondValueLength - 1) * 128;
                        lengthOffsize = 2;
                    }
                    obj.addProperty(key, processString(row, (keyValuePosition + lengthOffsize), valueLength));
                }
            ...
            throw new MySQLExtractException(
                    "Failure while extracting small json", e);
        }
        return obj;
    }


    private JsonArray parseSmallArray(byte[] row) throws MySQLExtractException {
        ...
        try {
            int keyNumber = LittleEndianConversion
                    .convert2BytesToInt(row, 0);
            for (int i = 0; i < keyNumber; i++) {
                int valueType = LittleEndianConversion
                        .convert1ByteToInt(row, 4 + 3 * i);
                int valueIndex = LittleEndianConversion
                        .convert2BytesToInt(row, 5 + 3 * i);

                if (valueType >= 0 && valueType <= 3) {
                    int valueLength = LittleEndianConversion
                            .convert2BytesToInt(row, valueIndex + 2);
                    ary.add(parseBinary(valueType, Arrays.copyOfRange(row, valueIndex, (valueLength + valueIndex))));
                } else if (valueType == 4) {
                    ary.add(parseLiteral(valueIndex));
                } else if (valueType == 5 || valueType == 6) {
                    ary.add(valueIndex);
                } else if (valueType >= 7 && valueType <= 11) {
                    ary.add((Number) parseValue(valueType, row));
                } else if (valueType == 12) {
                    int valueLength = LittleEndianConversion
                            .convert1ByteToInt(row, valueIndex);
                    int lengthOffsize = 1;
                    if (valueLength >= 128) {
                        int secondValueLength = LittleEndianConversion
                                .convert1ByteToInt(row, valueIndex + 1);
                        valueLength = valueLength + (secondValueLength - 1) * 128;
                        lengthOffsize = 2;
                    }
                    ary.add(processString(row, (valueIndex + lengthOffsize), valueLength));
                }
            }

        } catch (Exception e) {
            throw new MySQLExtractException(
                    "Failure while parse  small array", e);
        }
        return ary;
    }

    private static Object parseValue(int type, byte[] row) throws MySQLExtractException {
        Object value = 0;
        try {
            switch (type) {
                case 5:
                    value = LittleEndianConversion.convert2BytesToInt(row, 0);
                    break;
                case 6:
                    value = LittleEndianConversion.convert2BytesToInt(row, 0);
                    break;
                case 7:
                    value = LittleEndianConversion.convert4BytesToInt(row, 0);
                    break;
                case 8:
                    value = LittleEndianConversion.convert4BytesToInt(row, 0);
                    break;
                case 9:
                    value = LittleEndianConversion.convertSignedNBytesToLong(row, 0, 8);
                    break;
                case 10:
                    value = LittleEndianConversion.convert8BytesToLong(row, 0);
                    break;
                case 11:
                    value = MysqlBinlog.double8ToDouble(row, 0);
                    break;
                default:
                    break;
            }
            ...
            throw new MySQLExtractException(
                    "Failure while parse  json value", e);
        }
        return value;
    }

    private static int getValueLength(int type) {
        try {
            switch (type) {
                case 5:
                    return 2;
